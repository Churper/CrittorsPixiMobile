...
let initialClouds = 0;
  ...
  function setCurrentBirdHealth(health) {
    ...
    birdHpIndicator.style.setProperty('--hp-indicator-height', `${(1 - (getCurrentBirdHealth() / getBirdHealth())) * 100}%`);
  }
  ...
  function getNumberOfProjectiles() {
    return numberOfProjectiles;
  }

  function setNumberOfProjectiles(num) {
  ...
  function setBirdLevel(level) {
    birdLevel = level;
  }

  function getBirdLevel() {
    return birdLevel;
  }
  ...
  function setBirdSpeed(speed) {
    birdSpeed = speed;
  }

  function getBirdDamage() {
    return birdDamage;
  }

  function setBirdDamage(damage) {
    birdDamage = damage;
  }

  function getBirdHealth() {
    return birdHealth;
  }

  function setBirdHealth(health) {
    birdHealth = health;
  }
  ...
  var birdHeightPercentage = (1 - getCurrentBirdHealth() / getBirdHealth()) * 100;
  ...
  birdHPIndicator.style.setProperty('--current-health-bird', getCurrentBirdHealth());
  ...
  function getCharacterPortraitUrl(characterType) {
    switch (characterType) {
      ...
      case 'character-bird':
        return 'https://i.imgur.com/WAwZpGS.png';
      case 'character-frog':
...
if(getRoundOver() === true ){isAttacking = false;  attackAnimationPlayed = false; return;}
    if (!isAttackingChar) {
      if (!getisDead()) {
...
//console.log("HERXOROR:", getEnemiesInRange());
      ...
      isMoving = true;;
       ...
       // console.log("isatt:", isAttackingChar);
    //   console.log("isdead:", getisDead());
      //  console.log("iscombat:", isCombat);
      //  console.log("ispointerdown:", isPointerDown);
      //  console.log("enemiesinrange:", getEnemiesInRange());
...
//console.log("broke");
      ...
      const mountainSprites = [mountain1, mountain2, mountain3, mountain4];
  mountain3.tint = getRandomColor();
 mountain4.tint = getRandomColor();
 mountain1.tint = getRandomColor();
 mountain2.tint = getRandomColor();
      app.stage.addChild(background,mountain4, mountain1, mountain2, mountain3,  foreground, castle, critter, clouds, clouds2, hpBarBackground, hpBar, enemyDeath);
      ...
      setInterval(() => {
        if (!getisDead() && !getisPaused()) { 
      spawnPuffer(critter, pufferAttackTextures, enemyDeath, foreground, frogGhostTextures, pufferWalkTextures, frogGhostPlayer, frogWalkTextures);
...
const hpBarY = -(enemy.height / 3);
  ...
  let characterStats = {
    ...
    'character-bird': {
      speed: getBirdSpeed(),
      attack: getBirdDamage(),
      health: getBirdHealth(),
    },
    'character-bee': {
      speed: getBeeSpeed(),
      attack: getBeeDamage(),
